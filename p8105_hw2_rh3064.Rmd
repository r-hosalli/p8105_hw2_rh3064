...
The pols-month data is read in via `read_csv`. `separate()` is used to break the *mon* variable in *year*, *month* and *day*. *month.name* is a built-in constant that contains the English names for the months of the year, starting from January. The *month* variable is first coerced into a numeric vector, allowing it to be used inside the brackets to refer to the corresponding month name (e.g. if the month was 3, it would refer to the third element of *month.name* and be reassigned to 'March').
...
Next, the snp data set is imported and tidied in a similar manner as above.
